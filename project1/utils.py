...
class TargetColumnHandler:
    ...
    def move_target_to_end(self):
        ...
        cols = [c for c in self.df.columns if c != self.target_column_name] + [self.target_column_name]
...
class DataInvestigator:
    def __init__(self, df):
        ...
        self.target_data_type = self.df[self.target_col].dtype
    ...
    def check_model_type(self):
        if self.target_data_type in ['int64', 'float64']:
            if self.target_type == TargetType.CATEGORICAL:
                return "Classification could be applied as the target has limited unique numeric values."
            else:
        ...
        return "Unable to determine the model type. Inspect the target distribution."
    ...
    def summarize_dataset(self):
        ...
        summary = {
            ...
            "target_type": target_type,  # keep enum
            "unique_categories": unique_vals if target_type == TargetType.CATEGORICAL else None
        }
        ...
        if target_type == TargetType.CATEGORICAL:
            sentences.append(f"The target column has <b>{unique_vals} unique categories</b>.")

        return summary, sentences

    def perform_analysis(self):
        analyzer = ClassificationAnalyzer(self.df, self.target_col) if self.target_type == TargetType.CATEGORICAL else RegressionAnalyzer(self.df, self.target_col)
        return {
